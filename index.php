...
$app->post('/register', function ($request, $response, $args) use ($container) {
    $data = $request->getParsedBody();
    $user = User::register($container['db'], $data['username'], $data['mail'], $data['password'], $data['motto']);

    if ($user) {
        $response->getBody()->write('Inscription réussie !');
    } else {
        $response->getBody()->write('Une erreur est survenue pendant la création du compte');
    }
